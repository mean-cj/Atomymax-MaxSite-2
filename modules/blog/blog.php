<?
CheckUserblog($login_true, $pwd_login);
?>
<?include ("editor.php");?>
        ...
        <TR>
		  ...
		  &nbsp;&nbsp;<IMG SRC="images/menu/textmenu_blog.gif" BORDER="0"><BR>
				...
				<TR >
					...
					$res['mem'] = $db->select_query("SELECT * FROM ".TB_MEMBER." WHERE user='".$login_true."' ");
...
$db->connectdb(DB_NAME,DB_USERNAME,DB_PASSWORD);
$limit = 20 ;
$SUMPAGE = $db->num_rows(TB_BLOG,"id"," where  posted='".$login_true."' ");
$page=$_GET['page'];
if (empty($page)){
	$page=1;
}
...
$res['blog'] = $db->select_query("SELECT * FROM ".TB_BLOG." WHERE  posted='".$login_true."' ORDER BY id DESC LIMIT $goto, $limit ");
$count=0;
...
if ($FILE['name']) {
		...
		if (($FILE['type']!="image/jpg") AND ($FILE['type']!="image/jpeg") AND ($FILE['type']!="image/pjpeg")){
			...
			@copy ($FILE['tmp_name'] , "icon/blog_".TIMESTAMP."_".$login_true.".jpg" );
			$original_image = "icon/blog_".TIMESTAMP."_".$login_true.".jpg" ;
			$desired_width = _Iblog_W ;
			$desired_height = _Iblog_H ;
			$image = new hft_image($original_image);
			$image->resize($desired_width, $desired_height, '0');
			$image->output_resized("icon/blog_".TIMESTAMP."_".$login_true.".jpg", "JPG");
		}
	$pic='1';
} else {
	$pic='0';
...
?>
<FORM NAME="myform" METHOD=POST ACTION="?name=blog&file=blog&op=article_add&action=add" enctype="multipart/form-data">
<B><?=_FORM_TOPIC;?></B><BR>
...
?>
<FORM NAME="myform" METHOD=POST ACTION="?name=blog&file=blog&op=article_edit&action=edit&id=<?=$_GET['id'];?>" enctype="multipart/form-data">
<B><?=_FORM_TOPIC;?> </B><BR>
